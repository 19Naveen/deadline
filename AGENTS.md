...
Keyboard-driven terminal kanban board. Go 1.22+, three deps (`bubbles`, `bubbletea`, `lipgloss`).

```sh
go test ./...   # full suite
go vet ./...    # strict, keep clean
gofmt -l .      # must print nothing
go build -o ~/.local/bin/gotodo .   # install the binary under test
```
